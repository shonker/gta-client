...
bool renderer::imgui::elements::functions::begin_combo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10, 0), window->DC.CursorPos + ImVec2(308, 28 + 5));
    ...
    ImGui::RenderFrame(frame_bb.Min, frame_bb.Max - ImVec2(0, 5), ImColor(35, 42, 51), false, 4);

    if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
        ImGui::RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y - 5, frame_bb.Min.y + style.FramePadding.y + 5), ImColor(211, 211, 211, 211), ImGuiDir_Down, 1.0f);

    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + 10, frame_bb.Min.y + 6), ImColor(211, 211, 211, 211), preview_value);
    ...
    ImGui::SetNextWindowSizeConstraints(ImVec2(298.f, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
...
bool renderer::imgui::elements::functions::combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (::Button(item_text, { ImGui::GetWindowSize().x - 20, 25 }, item_selected))
        {
            value_changed = true;
            *current_item = i;
        }
